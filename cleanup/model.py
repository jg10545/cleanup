...
def get_distance_matrix(model, testdata):
    ...
    ----------
    model: sklearn Model 
        a trained sklearn RandomForestRegressor model
    testdata: numpy array
        an (N,d) numpy array of training data
    
    Returns
    -------
    numpy array
        an (N,N) array of pairwise distances.
    """
    # <--- snip
    N = testdata.shape[0]
    if N > 10000:
...
def random_forest_embeddings(X_train, X_test, Y_train, Y_test, n_estimators=100):
    ...
    ----------
    X_train: numpy array
        (N,d) numpy array of training covariates
    X_test: numpy array
        (N_t, d) numpy array of test covariates
    Y_train: numpy array
        (N,) numpy array of training labels
    Y_test: numpy array
        (N_t,) numpy array of test labels
    n_estimators: int, optional
        number of trees for the ensemble model
    
    Returns
    -------
    numpy array
        (N_t, 2) numpy array of T-SNE embeddings for each test point.
    numpy array
        (N_t,) numpy array of model prediction residuals for each test point.
    """
    # <-- snip
